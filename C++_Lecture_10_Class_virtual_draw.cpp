...
class IDrawer
{
	virtual void draw_line(Point pt1, Point pt2){cout<<"IDrawer::draw"<<endl;}
}
...
public:
	...
	void circle(Point pt1, size_t radius) override
	{
		...
		std::cout << "pt1=" << pt1.x << "," << pt1.y << " O " << "radius=" << radius << std::endl;
	};
	void rectangle(Point pt_tl, Point pt_br) override
	{
		cout << "DrawTxt::rectangle" << endl;
		std::cout << "pt1=" << pt_tl.x << "," << pt_tl.y << " �� " << "pt2=" << pt_br.x << "," << pt_br.y << std::endl;
	};
	...
	void txt(Point pt, std::string msg) override
	{
		...
		std::cout << "pt=" << pt.x << "," << pt.y << " hello world c++ " << "msg=" << msg << std::endl;
	};
};
...
public:
	...
	DrawMat(const Mat& color_img) :draw_img(color_img) { cout << "DrawMat::Ctor" << endl; }
	...
	void txt(Point pt, std::string msg) override
	{
		...
		Scalar color = Scalar(rng.uniform(0, 256), rng.uniform(0, 256), rng.uniform(0, 256));
		cv::putText(draw_img, msg, Point(pt.x, pt.y + textsize.height), fontFace, fontScale, color, thickness, cv::LINE_8);
	};
};

...
def face_procs(face_image, face_cascade, eye_cascade, russian_cascade):
    ...
    russian_plates = russian_cascade.detectMultiScale(gray_img, 1.1, 5, minSize=(30, 30))

    # 번호판 주위에 사각형 그리기
    for (x, y, w, h) in russian_plates:
...
russian_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_russian_plate_number.xml')
...
if mode_video:
    while True:    
        ...
        ret, frame = cap.read()
        if ret:
            face_procs(frame, face_cascade, eye_cascade, russian_cascade)
